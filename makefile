...
build: 
	...
	@echo "Configuring Dockerfile.....!!!!"; sed -i.bck -e 's/<TERRAFORM_VERSION>/$(TERRAFORM_VERSION)/g' -e 's/<SHA256SUM>/$(SHA256SUM)/g' Dockerfile;
	@docker build --no-cache -t $(DOCKER_NAME):latest -t $(DOCKER_NAME):$(TAG_NAME) .
	@mv Dockerfile.bck Dockerfile; rm -rf Dockerfile.bck
